...
| **la-fragua** (RK3588, user **`ubuntu`**) | Batch/embeddings/Qdrant/Nexo, siempre encolado (`p0x-enqueue`), térmica vigilada | Corre también "El Faro" (servicio HEXELION de atestación AIS, firma ed25519 — fuera de alcance de `soberano`) |
...
### Cada nodo tiene SU usuario, y `~` no significa lo mismo en los dos

`soberano` es **`pisky`**; `la-fragua` es **`ubuntu`**; `la-torre` es **`jetson`**.
Lo dice `~/.ssh/config` de este nodo y es donde se comprueba, no de memoria.

Importa porque casi todo lo que sale de aquí hacia otro nodo es **propose-only**:
un artefacto que alguien va a pegar *allí*. Y en ese artefacto **`~` y `$HOME` se
expanden al usuario que ejecuta, no al que escribió**. Una ruta con `~` escrita
desde `soberano` y aplicada en `la-fragua` apunta a otro sitio sin avisar, y falla
callando o —peor— escribe donde no toca.

**La regla, entonces:** en un artefacto propose-only las rutas del nodo de destino
van **absolutas y con su usuario dentro**, nunca con `~`. Y si lleva una unidad
systemd de sistema, lleva `User=` y `Group=` explícitos.

**Y el reverso, que es el error simétrico:** una unidad `--user` del propio
`soberano` **no admite `User=` ni `Group=`** —systemd los rechaza en unidades de
usuario— y usa `%h`, que ya se expande al home correcto. Ponerle el usuario de
otro nodo «por si acaso» la rompe. Antes de tocar un usuario en un fichero,
preguntar en qué nodo corre eso.
